...
do_a_survey <- function(simexp.df, sim.k,
                        ...
                        model_names,dataname,targfolder,
                        hm_ppnts,
...
datalist <<- list(hm_trials=nrow(simexp.df),
        ...
        destination <- paste0(targfolder,"/calc",calcsd_level,"ord",ordsd_level,"tolerance",tolerance_level,"model",modelname,"data",dataname,"fit.RData")
        ...
        save.image(file=paste0(targfolder,"/calc",calcsd_level,"ord",ordsd_level,"tolerance",tolerance_level,"model",modelname,"data",dataname,"fit.RData"))
...
for(adataset in list.files("stimsets/")){
    print("Choices for")
    print(adataset)
    
    simexp.df <- read.csv(paste0("stimsets/",adataset))#There must be a cleaner idiom for this! Sheesh. TODO, loop over this list.
    load("simk.RData")
    
do_a_survey(simexp.df, sim.k,
                        calcsd_levels=c(.15),ordsd_levels=c(.15),tolerance_levels=c(.1), #arbitrary but probably pretty important!
                        model_names=c("getchoices.stan"),dataname=substr(adataset,1,nchar(adataset)-4),targfolder="simexp",
                        hm_ppnts=nrow(sim.k),
                        hm_options=3, #can change to 2 if you want, extra options will just be ignored if present.
                        hm_attributes=2 #Must match the stim df provided, it's up to you to stay consistent.
            )
    rm(list=setdiff(ls(),c("adataset","do_a_survey")));gc(); #possibly unneccessary, but stan fits in a loop is hazardous, memorywise.
}
