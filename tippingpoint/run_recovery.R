library(tidyverse)
library(rstan)
library(shinystan)
rm(list=ls())
options(mc.cores = parallel::detectCores())
rstan_options(auto_write = TRUE)


recover_k <- function(targdata,modelstring,dataname){
load(targdata) #or whatever it is
##simexp.df$simchoice <-   as.numeric(mysamples%>%dplyr::select(contains("choice"))%>%sample_n(1)%>%t)#delete
if(file.exists(paste0(modelstring,dataname,"recovery.RData"))){
    print("Skip existing:")
    print(paste0(modelstring,dataname,"recovery.RData"))
    return;
}

hm_options = 3; #Should not have to set this here. Don't forget it's there!
hm_attributes=2; #Why on earth is this getting reset here? Ugh.
hm_ppnts = length(unique(simexp.df$ppntid));

ttoa = rep(NA,nrow(simexp.df)*hm_options*hm_attributes)
dim(ttoa) <- c(nrow(simexp.df),hm_options,hm_attributes)#convert to matrix
for(atrial in 1:nrow(simexp.df)){ #populate with values
    for(i in 1:hm_attributes){
        for(j in 1:hm_options){
            ttoa[atrial,j,i] <- simexp.df[atrial,paste0("option",j,"attribute",i)]
        }
    }
}

datalist = list(hm_trials=nrow(simexp.df),
                hm_ppnts=hm_ppnts,
                ppntid=simexp.df$ppntid,

                hm_options=hm_options,
                hm_attributes=hm_attributes,

                truth_trial_option_attribute = ttoa,

                choice=simexp.df$choice,

                calcsd = rep(.15,hm_ppnts),
                ordsd = rep(.15,hm_ppnts),
                tolerance= rep(.1,hm_ppnts)

                )

recovery.fit <- stan(file=paste0(modelstring,".stan"),#"baseline_regression.stan",#file="agentRecovery_konly.stan",
           data=datalist,
           iter=1000,
           chains=4,
           init=function(){
               zeros <- rep(0,nrow(simexp.df)*hm_options*hm_attributes)
               dim(zeros)=c(nrow(simexp.df),hm_options,hm_attributes)
               return(
                   list(est_trial_option_attribute=zeros#,
                        ## calcsd=rep(.1,hm_ppnts), #might need to revisit these inits?
                        ## ordsd=rep(.1,hm_ppnts),
                        ## tolerance=rep(.1,hm_ppnts)
                        )
               )
           },
           control = list(max_treedepth = 13,adapt_delta = 0.85) # feasible? 10 and .8 defaults are too low, fifteen and .9 too high
           );

recovery.fit <- list(fit=recovery.fit,data=simexp.df)
#recovery.samples <- as.data.frame(extract(recovery.fit, permuted = TRUE)) # extract returns a list of arrays
save(recovery.fit,file=paste0(modelstring,dataname,"recovery.RData"))#file="baseline_recovery.RData")#file="recoveryfit.RData")
#View("done")
}#ends recover k


for(targdata in list.files("simexp")){
    dataname = strsplit(targdata,"getchoices.stan")[[1]][2]%>%strsplit(".RData") #should be able to just split on two delimiters. Whatever.
    recover_k(paste0("simexp/",targdata),"recovery/baseline",dataname)
    recover_k(paste0("simexp/",targdata),"recovery/hybrid",dataname)
#    recover_k(paste0("simexp/",targdata),"recovery/purehowes",dataname)#Slow & bad diagnostics, better go with hybrid.
    
    gc()
}
##recover_k(targdata,"agentRecovery_konly") # this particular hybrid is not the only howes16-influenced option, might not be the best choice. Consider.
