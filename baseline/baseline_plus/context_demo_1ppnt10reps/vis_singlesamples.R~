...
for(afile in c("calc0.15ord0.15tolerance0.3modelgetchoices.stanfit.RData","calc0.15ord0.15tolerance0.05modelgetchoices.stanfit.RData","calc0.15ord0.15tolerance0.161111111111111modelgetchoices.stanfit.RData")){ #biggest, smallest, and just over switchpoint. was: list.files(pattern="^calc.*RData")){

    load(afile)

    targfolder=paste0("single_sample/",strsplit(afile,".RData")[1],"plots")
...
#            geom_line(data=data.frame(x=c(0,1),y=c(1,0)),aes(x=x,y=y),alpha=.9,color="grey",linetype="dotted",size=2)+
    ...
    geom_line(data=calcobs.df,aes(x=x,y=y3),color="green")
    }

    ordprobs <- thissample%>%select(contains("ordprob_tracker"))

    for(trialnumber in 1){ #unique(simexp.df$trialid)
        ...
        ggsave(statusprobs.plot+(stim.plot(trialnumber)+geom_text(aes(x=2,y=2,label=paste("chose",c("red","blue","green")[thischoice],"(happens ",overall.df[thischoice,"count"],")"))))+plot_layout(ncol=1), #endorsement.plot silly with one sample
