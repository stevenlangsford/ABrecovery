...
#            geom_line(data=data.frame(x=c(0,1),y=c(1,0)),aes(x=x,y=y),alpha=.9,color="grey",linetype="dotted",size=2)+
    ...
    for(trialnumber in 1){ #unique(simexp.df$trialid)
        ...
        ggsave(stim.plot(trialnumber)+geom_text(aes(x=2,y=2,label=paste("chose",c("red","blue","green")[thischoice],"(happens ",overall.df[thischoice,"count"],")"))))+plot_layout(ncol=1), #endorsement.plot silly with one sample
