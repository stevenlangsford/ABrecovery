...
model{
  ...
  for(atrial in 1:hm_trials){
    for(anoption in 1:hm_options){
      estval[atrial,anoption]~normal(truth_trial_option_attribute[atrial,anoption]*k[ppntid[atrial]], calcsd[ppntid[atrial]]);//engine of the baseline version transplanted
      for(anattribute in 1:hm_attributes){
	est_trial_option_attribute[atrial,anoption,anattribute]~normal(0,1);
      }
    }
  }
  /* //apply the calculation observation: this is sanscalc */
  /* for(atrial in 1:hm_trials){ */
  /*   for(anoption in 1:hm_options){ */
