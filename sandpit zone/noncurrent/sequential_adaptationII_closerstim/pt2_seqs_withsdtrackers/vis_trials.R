...
afile="modelgetchoices.stanfit.RData"
for(afile in list.files(pattern=".*RData")){
    ...
    stim.plot <- function(trialnumber){
        ...
        optionnoise <- list("Targ"=mysamples[1,paste0("calcsd_tracker.",trialnumber,".",1)],
                            "Comp"=mysamples[1,paste0("calcsd_tracker.",trialnumber,".",2)],
                            "Decoy"=mysamples[1,paste0("calcsd_tracker.",trialnumber,".",3)])
...
for(anoption in 1:3){
    ...
    ggplot(simexp.df[trialnumber,])+
        ...
        guides(color=FALSE)+
        theme_bw()+ggtitle(paste(names(optionnoise),as.character(optionnoise), collapse=" ")) #+xlim(c(0,1))#+ylim(c(0,1))
    }
