...
model{
  ...
  for(atrial in 1:hm_trials){
	...
	est_trial_option_attribute[atrial,anoption,anattribute]~normal(100,50); //attribute priors: was N(0,1). Matches triangles? check. Note same prior both attributes.
      }
    }
  }
...
generated quantities{
  ...
  for(atrial in 1:hm_trials){
    for(anoption in 1:hm_options){
      estval_tracker[atrial,anoption]= (est_trial_option_attribute[atrial,anoption,1]*est_trial_option_attribute[atrial,anoption,2]);//maybe *10,^5 extremification
    }
