...
##Convenient for R
triad_to_df <- function(atrial,shapeflavor,role, orientation, stimid){
...
for(decoydist in c(0.95,0.9,0.85)){#5,10,15% gap on decoys
    for(shapeflavor in list(c(0,0,0),c(0,0,1),c(0,1,0),c(1,0,0),c(0,1,2))){
        ...
        stimlist <- append(stimlist,triad_to_js(atrial,shapeflavor,c("targ","comp","decoy"),c(0,1,0),stimid))
        stim.df <- rbind(stim.df,triad_to_df(list(targ,get_competitor(targ),decoy),shapeflavor,c("targ","comp","decoy"),c(0,1,0),stimid))#notice attributes here match 'NS' and 'EW', not base-and-height+orientation, which is how the js draws them. Beware confusion, the meaning of orientation has changed. This is not ideal...
    }
}
...
for(slidedist in c(-.025,-.05,-.1,-.15)){
    for(shapeflavor in list(c(0,0,0),c(0,0,1),c(0,1,0),c(1,0,0),c(0,1,2))){
        ...
        stimlist <- append(stimlist,triad_to_js(list(targ,targ,decoy),shapeflavor,c("targ","comp","decoy"),c(0,1,0),stimid))
        stim.df <- rbind(stim.df,triad_to_df(list(targ,get_competitor(targ),decoy),shapeflavor,c("targ","comp","decoy"),c(0,1,0),stimid))#notice attributes here match 'NS' and 'EW', not base-and-height+orientation, which is how the js draws them. Beware confusion, the meaning of orientation has changed. This is not ideal...
    }
}
