...
load("calc0.2ord0.1tolerance0.1modelgetchoices_stimaware.stanfit.RData")
...
datalist = list(hm_trials=nrow(simexp.df),
                ...
                calcsd = rep(.2,hm_ppnts),
                ordsd = rep(.2,hm_ppnts),
                tolerance= rep(.1,hm_ppnts)

                )

recovery.fit <- stan(file="agentRecovery_konly_stimaware.stan",
           data=datalist,
           iter=1000,
           chains=4,
           init=function(){
...
ggsave(ggplot(recoverycheck.df,aes(x=sim.1,y=recovered.1))+geom_point()+theme_bw(),file="krecovery.png")
