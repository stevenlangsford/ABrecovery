...
#    browser();
    for(trialnumber in 1:nrow(simexp.df)){ #long slow version, plot every trial.
        print(paste(trialnumber,"of",nrow(simexp.df),sep=" "))
        ...
        endorsement.plot <- (ggplot(choicesummary.df,aes(x=optionchosen,y=count,fill=optionchosen))+geom_bar(stat="identity")+scale_fill_manual(values=fixedcolors)+theme_bw()+xlab("")+ guides(fill=FALSE))+ggtitle(paste("trial number ",trialnumber,                                                                                                                                       "ppnt",simexp.df[trialnumber,"ppntid"],
                                                                                                                                                                                                                           "k",sim.k[simexp.df[trialnumber,"ppntid"],1],
                                                                                                                                                                                                                           sep=" "
                                                    ))
