...
#            geom_line(data=data.frame(x=c(0,1),y=c(1,0)),aes(x=x,y=y),alpha=.9,color="grey",linetype="dotted",size=2)+
    ...
    for(trialnumber in 1:nrow(simexp.df)){
        ...
        endorsement.plot <- (ggplot(choicesummary.df,aes(x=optionchosen,y=count,fill=optionchosen))+geom_bar(stat="identity")+scale_fill_manual(values=fixedcolors)+theme_bw()+xlab("")+guides(fill=FALSE))+ggtitle(paste("ppnt",simexp.df[trialnumber,"ppntid"],"trial",trialnumber))
