...
model{
  ...
  for(atrial in 1:hm_trials){
	...
	if(anattribute % 2 == 0) est_trial_option_attribute[atrial,anoption,anattribute]~normal(2,.5); //Symmetry-breaker allows compromise effect?
	else est_trial_option_attribute[atrial,anoption,anattribute]~normal(2,.5); //?
      }
    }
  }
