...
#    simexp.df$trialid=1:nrow(simexp.df)
    ...
    plotdir = paste0(gsub(".RData","",afile),"plots");
    dir.create(plotdir)
    ...
    for(i in 1:length(unique(simexp.df$trialid))){ #1:4 # 5:8 #9:12 #13:16
        if(exists("twooption.samples")){
            ggsave(
                stim.plot(i)/
                (
                    (trialeval.plot(i,mysamples,3))+
                    (trialeval.plot(i,twooption.samples,2))

                )
               ,file=paste0(plotdir,"/trial",i,".png"),width=15)}else{

                                                                 ggsave(
                                                                     stim.plot(i)/
                                                                     (trialeval.plot(i,mysamples,3)),
                                                                     file=paste0(plotdir,"/trial",i,".png"),width=15)
                                                             }
    }
}
##inspect aggregate
##rm(list=setdiff(ls(),c("twooption.samples","mysamples"))) #just for fun.
...
if(saveplots){
   ...
   ,file=paste0(plotdir,"/contextstim.png"),width=15)
}
