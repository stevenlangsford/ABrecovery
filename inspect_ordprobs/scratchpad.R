...
trialnumber=1
stim.plot(trialnumber)


## ggplot()+geom_point(
##              data=estcloud.df%>%group_by(option)%>%summarize(x=mean(x),y=mean(y))%>%as.data.frame,
##              aes(x=x,y=y,color=as.factor(option)),alpha=1,size=5,shape=2)
